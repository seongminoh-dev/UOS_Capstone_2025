...
fn cs_main(@builtin(global_invocation_id) ThreadID : vec3<u32>)
{
    ...
    let motionVector : vec2<u32> = vec2<u32>(u32(motionVector_raw.x) , u32(motionVector_raw.y));
    let prevPixel : vec2<u32> = vec2<u32>(curPixel - motionVector);
    let resi        : vec2<u32> = vec2<u32>(UniformBuffer.Resolution_Source);

    if (!(all(prevPixel>= vec2<u32>(0, 0)) && all(prevPixel <  resi))) {
        ReservoirBuffer[curIdx] = baseRes;
        return;
    }
    ...
    // offset path의 Jacobian 계산 & 유효성 체크
    ...
    if (rnd < p_choose_off) {
        ...
        outRes.UCW = baseRes.Sample.J/det_J;
    } 
