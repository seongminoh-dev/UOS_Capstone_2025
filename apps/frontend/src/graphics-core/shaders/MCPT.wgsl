...
fn cs_main(@builtin(global_invocation_id) ThreadID : vec3<u32>)
{
    ...
    if ( !CSurface[1].IsValidSurface )
    {
        let CameraRay : Ray = GenerateRayFromThreadID( ThreadID.xy );
        Color = GetEnvironmentColor( CameraRay.Direction );

        textureStore(ResultTexture, ThreadID.xy, vec4<f32>(Color, 1.0));
        return;
    }
